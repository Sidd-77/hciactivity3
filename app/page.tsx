...
export default function Component() {
      ...
      case 'classrooms':
        results = typedUniversityData.classrooms.filter(classroom => {
          ...
          const capacityMatch = (!capacityRange.min || classroom.capacity >= parseInt(capacityRange.min)) &&
                                (!capacityRange.max || classroom.capacity <= parseInt(capacityRange.max))
      ...
      case 'departments':
        results = typedUniversityData.departments.filter(department => {
          ...
          const budgetMatch = (!budgetRange.min || department.budget >= parseInt(budgetRange.min)) &&
                              (!budgetRange.max || department.budget <= parseInt(budgetRange.max))
      ...
      case 'instructors':
        results = typedUniversityData.instructors.filter(instructor => {
          ...
          const salaryMatch = (!salaryRange.min || instructor.salary >= parseInt(salaryRange.min)) &&
                              (!salaryRange.max || instructor.salary <= parseInt(salaryRange.max))
          ...
          <div className="space-y-4">
            ...
            <div className="flex space-x-2">
              <Input 
                ...
                onChange={(e) => setCreditRange(prev => ({ ...prev, min: e.target.value }))}
              />
              <Input 
                ...
                onChange={(e) => setCreditRange(prev => ({ ...prev, max: e.target.value }))}
              />
            </div>
          </div>
        )
    ...
    switch (activeSearch) {
      case 'classrooms':
        const classroomData = buildings.map(building => {
          ...
          return {
            ...
            mediumRooms: rooms.filter(r => r.capacity > 25 && r.capacity <= 50).length,
        ...
        return (
          <Card className="bg-card border border-muted">
            <CardHeader>
              <CardTitle className="text-primary">Classroom Distribution by Building</CardTitle>
              <CardDescription>Overview of room sizes across buildings</CardDescription>
            </CardHeader>
            <CardContent>
              <ChartContainer config={{
                smallRooms: { label: "Small Rooms", color: "hsl(var(--chart-1))" },
                mediumRooms: { label: "Medium Rooms", color: "hsl(var(--chart-2))" },
                largeRooms: { label: "Large Rooms", color: "hsl(var(--chart-3))" },
              }} className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={classroomData}>
                    <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--muted))" />
                    <XAxis dataKey="building" stroke="hsl(var(--muted-foreground))" />
                    <YAxis stroke="hsl(var(--muted-foreground))" />
                    <ChartTooltip content={<ChartTooltipContent />} />
                    <Legend />
                    <Bar dataKey="smallRooms" stackId="a" fill="hsl(var(--chart-1))" />
                    <Bar dataKey="mediumRooms" stackId="a" fill="hsl(var(--chart-2))" />
                    <Bar dataKey="largeRooms" stackId="a" fill="hsl(var(--chart-3))" />
                  </BarChart>
                </ResponsiveContainer>
              </ChartContainer>
              <div className="mt-4">
              ...
              {selectedDataPoint && (
                <div className="mt-4 p-4 border border-muted rounded-md bg-secondary">
                  <h3 className="font-bold text-primary">{selectedDataPoint.building}</h3>
      ...
      case 'departments':
        return (
          <Card className="bg-card border border-muted">
            <CardHeader>
              <CardTitle className="text-primary">Department Budget and Size Comparison</CardTitle>
              <CardDescription>Overview of department budgets, faculty, and students</CardDescription>
            </CardHeader>
            <CardContent>
              <ChartContainer config={{
      ...
      case 'courses':
        return (
          <Card className="bg-card border border-muted">
            <CardHeader>
              <CardTitle className="text-primary">Course Enrollment and Credits</CardTitle>
              <CardDescription>Overview of course enrollment and credit hours</CardDescription>
            </CardHeader>
            <CardContent>
              <ChartContainer config={{
                enrollment: { label: "Enrollment", color: "hsl(var(--chart-1))" },
              ...
              {selectedDataPoint && (
                <div className="mt-4 p-4 border border-muted rounded-md bg-secondary">
                  <h3 className="font-bold text-primary">{selectedDataPoint.title}</h3>
          ...
          <Card className="bg-card border border-muted">
            ...
            <CardContent>
              <ChartContainer config={{
                ...
                publications: { label: "Publications", color: "hsl(var(--chart-2))" },
              }} className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <ScatterChart>
                    <CartesianGrid stroke="hsl(var(--muted))" />
  ...
  return (
    <div className="container mx-auto p-8 bg-background text-foreground border-muted">
      <h1 className="text-4xl font-bold mb-8 text-primary">University Information System</h1>
                ...
                <TableHeader>
                  <TableRow className="border-b border-muted">
                    {searchResults.length > 0 && Object.keys(searchResults[0]).map((key) => (
                      <TableHead key={key} className="text-muted-foreground border-r border-muted last:border-r-0">{key}</TableHead>
                    ))}
                  </TableRow>
                </TableHeader>
                <TableBody>
      ...
      <div className="mt-12">
        <h2 className="text-3xl font-bold mb-6 text-primary">Data Visualization</h2>
        {renderVisualization()}
      </div>
    </div>
  )
